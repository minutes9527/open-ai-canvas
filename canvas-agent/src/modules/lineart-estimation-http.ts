...
export function resolveLineartRuntimePaths(env: NodeJS.ProcessEnv = process.env): LineartRuntimePaths {
    ...
    const projectRoots = [
        configuredProjectRoot ? resolveRuntimePath(configuredProjectRoot) : undefined,
        path.resolve(moduleRoot, ".."),
        path.resolve(process.cwd()),
    ].filter((value): value is string => Boolean(value));
    const projectRoot = configuredProjectRoot
        ? resolveRuntimePath(configuredProjectRoot)
    ...
    const pythonCandidates = env.CANVAS_DEPTH_PYTHON?.trim()
        ? [resolveRuntimePath(env.CANVAS_DEPTH_PYTHON)]
    ...
    const hfHome = resolveRuntimePath(env.CANVAS_LINEART_HF_HOME?.trim() || env.CANVAS_DEPTH_HF_HOME?.trim() || path.join(projectRoot, ".local", "cache", "huggingface"));
