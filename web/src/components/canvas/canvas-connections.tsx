...
export function connectionHandleY(node: CanvasNodeData, handleId?: string, scrollTop = 0, anchorRatio?: number) {
    if (handleId?.startsWith("row:") && node.metadata?.frameScriptStoryboard && anchorRatio !== undefined) {
        return node.position.y + node.height * Math.min(1, Math.max(0, anchorRatio));
    }
