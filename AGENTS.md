...
`open-ai-canvas` 是 AI 创意生产工作台，不是简单的聊天工具。核心创作链路是：

`故事构思 -> 分镜设计 -> 角色资产 -> 图片生成 -> 视频生成 -> 创作管理`

产品方向参考 Figma 的无限画布与协作设计、Adobe Premiere 的专业创作流程、Runway 的 AI 视频创作，以及 Midjourney Canvas 的 AI 图像工作流。当前优先级依次是：Canvas 无限画布体验、分镜创作流程、AI Agent 工作流、角色和素材资产管理、专业创作者操作体验。最终目标是成为可实际用于 AI 影视创作生产的专业工具。

Codex 负责代码实现、架构维护、组件与功能开发、Bug 修复、性能优化和测试验证。产品方向和重要 UI/UX 决策由设计评审流程确定；未经用户明确要求，不自行大幅改变产品交互逻辑。

### UI/UX 原则

- 整体保持专业创作工具、深色工作环境、高信息密度、画布优先、控制面板辅助的方向。
- 避免普通 SaaS 后台式页面、大量卡片堆叠、过度弹窗和简单聊天机器人式界面。
- 开发页面前先明确用户目标、主工作流、下一步操作和信息层级；重要内容直接呈现，次要内容按需隐藏。
- 不只实现视觉效果；必须确认数据来源、页面位置、交互闭环及对现有功能的影响。
- 页面或重要功能完成后，准备页面截图、修改说明和主要交互说明，交给设计评审。
- 完成代码不等于产品验收结束；继续检查是否符合创作工具定位、是否降低操作成本、是否具有专业产品质感。

### 组件与实现原则

- 优先复用已有组件和设计系统，不重复创建相似组件；保持组件职责单一并保留合理扩展能力。
- Canvas 能力按职责拆分为 `Canvas`、`Node`、`Toolbar`、`PropertyPanel`、`AssetPanel`、`Timeline` 等边界，避免将所有状态和交互堆入单个文件。
- 保持现有技术栈，优先最小且可维护的修改，不破坏已有功能。

### Git 与交付流程

- 不直接在 `main` 上开发。使用 `codex/` 前缀的功能分支，例如 `codex/canvas-ui-redesign`、`codex/storyboard-feature`、`codex/agent-workflow`。
- 每个完整功能使用清晰的提交说明，例如 `feat: redesign canvas property panel`、`fix: resolve image model selection issue`、`refactor: improve asset management component`；仓库已有更严格提交格式时，以第 9 节为准。
- 每次完成任务后说明修改文件、新增组件、删除内容、页面与用户体验变化、已运行的测试和已知问题。
